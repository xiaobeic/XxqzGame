...
public class story extends JPanel{
	...
	public static BufferedImage beijing2;
	...
	public static BufferedImage slei,sxin,sxiang,ssu,stiao,sfang,shuo,entrench,syin,yinshen;
	public static BufferedImage huojian1,huojian2,huojian3,huojian4;
	...
	static{             //加载图片
		try{
			...
			beijing2=ImageIO.read(Move.class.getResource("beijing2.png"));
			...
			syin=ImageIO.read(Move.class.getResource("syin.png"));
			yinshen=ImageIO.read(Move.class.getResource("yinshen.png"));
	...
	public void paint(Graphics g)
	 {
		if(focu==1){
			...
			if(y8<-10)
	   		...
	   		if(i8<170&&y8==137||i8<95&&y8==227||i8<320&&y8==317||i8<95&&y8==404||i8<170&&y8==491){           //恐怖分子在边界自动转体
				a8=1;
				left8=0;
				right8=1;
	   			i8+=2;
	   		}
			if(i8>800&&y8==137||i8>890&&y8==227||i8>690&&y8==317||i8>890&&y8==404||i8>800&&y8==491){
				a8=0;
				left8=1;
				right8=0;
				i8-=2;
			...
			if(cca8>=1){
			 ...
			 if(y<y8&&yins==0){//处理上键           e.getKeyCode() == KeyEvent.VK_UP
				 shift=0;
				 if(rocket8==0){
					 if(aUP8==0&&j8ump<1||aUP8>40&&j8ump<1){
						 j8ump++;
						 aUP8=1;
			...
			if(shift>150&&usm>400&&y>0){                   //处理恐怖分子自动移动按键间隔
				if(y>y8&&yins==0){//处理下键                                   e.getKeyCode() == KeyEvent.VK_DOWN
					if(aDOWN8==0||aDOWN8>40){
						shift=0;
						aDOWN8=1;
						y8+=3;
						if(y8==494)                       //处理最下层按下键不会掉落
							y8-=3;
						if(y8==407)                         //处理最下层按下键不会掉落
							if(i8>85&&i8<160||i8>810&&i8<900)            
								y8-=3;
						new  music("music/down.wav");          //跳跃的声音
						music.ac.play();
					}
				}
				 if(i<i8&&i>80&&y8>0&&yins==0){//处理左键               //e.getKeyCode() == KeyEvent.VK_LEFT
					 shift=0;
					 ri8=0;
					 ri++;
					 if(ri==1){ 
				 ...
				 if(i>i8&&i<900&&y8>0&&yins==0){//处理右键             e.getKeyCode() == KeyEvent.VK_RIGHT
					 shift=0;
					 ri=0;
					 ri8++;
					 if(ri8==1){
				...
				if(y>y8){               //有火箭时处理不一直飞行
				 ...
				 if(y>500||y<-10){//处理人物停止移动                 e.getKeyCode() == KeyEvent.VK_NUMPAD3
						 left8=0;//控制按下左右移动
						 right8=0;
						 ri=ri8=0;
				 }
				 if(y==y8&&(i8-i)>450&&a==1&&a8==0&&bb8!=0&&leia8==0&&leib8==0&&usm>640){           //扔雷       e.getKeyCode() == KeyEvent.VK_ADD &&bb8!=0&&leia8==0&&leib8==0
					 if(a8==0)
						 leia8=1;
					 if(a8==1)
						 leib8=1;
				 ...
				 if(y==y8&&(i-i8)>450&&a==0&&a8==1&&bb8!=0&&leia8==0&&leib8==0&&usm>640){           //扔雷       e.getKeyCode() == KeyEvent.VK_ADD &&bb8!=0&&leia8==0&&leib8==0
					 if(a8==0)
						 leia8=1;
					 if(a8==1)
						 leib8=1;
				...
				if(cc8==0&&cc82!=0){//换枪                    e.getKeyCode() == KeyEvent.VK_DECIMAL
				...
				if(cc82==0&&cc8==6){//换枪                    e.getKeyCode() == KeyEvent.VK_DECIMAL
				 ...
				 if(y==y8&&usm>640){//处理子弹键             e.getKeyCode() == KeyEvent.VK_ENTER&&usm>640
			...
			if(usm>120){
				g.drawImage(beijing2,0,-15,1100,700,null);      //首先画
				...
				if(buji==0){                             //补给
					...
					if(song==1||song==2||song==3||song==4||song==5||song==6||song==7||song==8){
			 	...
			 	if(song==1){
		 			...
		 			if(bujitime<1200)
		 				g.drawImage(syin,songx,songy+20,null);
		 			if(bujitime>=1200){
		 				buji=0;
			 			song=9999;
			 			songy=-100;
				...
				if(yins!=0){
					yins++;
					g.drawImage(yinshen,i-50,y-40,180,180,null);
					if(yins==1500)
						yins=0;
				}
				if(yins8!=0){
					yins8++;
					g.drawImage(yinshen,i8-50,y8-40,180,180,null);
					if(yins8==1500)
						yins8=0;
				}
	...
	//人物1	
			...
			if(i>=1000)
				i-=8;
			if(y==140){
				if(i>=160&&i<=810)       //处理在台阶上不下落，不在台阶上自动下落
					y-=3;
				else{
					y+=3;
					if(rocketup1==1)
...
//人物2			
			...
			if(i8>=1000)
				i8-=8;
			if(y8==140){
				if(i8>=160&&i8<=810)       //处理在台阶上不下落，不在台阶上自动下落
					y8-=3;
				else{
					y8+=3;
					if(rocketup81==1)
			 ...
			 {  
				...
				if(e.getKeyCode() == KeyEvent.VK_S){//处理下键
					if(aDOWN==0||aDOWN>40){					
						aDOWN=1;
						y+=3;
						if(y==494)                       //处理最下层按下键不会掉落
							y-=3;
						if(y==407)                         //处理最下层按下键不会掉落
							if(i>85&&i<160||i>810&&i<900)            
								y-=3;
						new  music("music/down.wav");          //跳跃的声音
						music.ac.play();
					}
				 ...
				 if(e.getKeyCode() == KeyEvent.VK_D){//处理右键
					 le=0;
					 le1++;
					 if(le1==1){
						 i+=10;
				 ...
				 if(e.getKeyCode() == KeyEvent.VK_W&&y>0){//处理上键
					 if(rocket==0){
